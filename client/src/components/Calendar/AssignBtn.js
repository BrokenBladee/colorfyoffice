...
export class AssignBtn extends Component {
  ...
  onSubmit = (e) =>{
    e.preventDefault();
    this.props.assignN(this.state.firstName, this.state.lastName, this.state.reason2);
  ...
  render() {
    return (
      <form onSubmit={this.onSubmit} className= "flex flex-col min-w-full max-w-1 sm:max-w-none">
        ...
        <select 
          ...
          {this.state.reason.map((reason)=>(
            <option key={reason.id} reason={reason}>{reason.reasonName}</option>
          ))}
